...
const main = async () => {
  ...
  const requester = await deployContract(`contracts/Requester.sol`, [airnodeRrp.address]);
...
export const makeRequest = async (): Promise<string> => {
  ...
  const endpointId = readConfigAirnode().triggers.rrp[0].endpointId;
  // NOTE: When doing this manually, you can use the 'derive-sponsor-wallet-address' from the admin CLI package
  ...
  const { getEncodedParameters } = await import(`../airnode-deployment/request-utils`);
